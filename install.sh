...
show_summary() {
    ...
    echo "  1. 在 .env 中配置 ALERTMANAGER_URL 指向外部 Alertmanager"
    echo "  2. 确保 Oracle 数据库可访问"
    echo "  3. 确保本服务可以访问 Alertmanager (网络/防火墙)"
    echo ""
    echo -e "${CYAN}Prometheus 集成:${NC}"
    echo "  在 Prometheus 配置中添加抓取任务:"
    echo "    - job_name: 'zmc-alarm-exporter'"
    echo "      static_configs:"
    echo "        - targets: ['本机IP:${SERVER_PORT:-8080}']"
