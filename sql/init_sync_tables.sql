...
--
-- 架构说明 (v2.0):
-- 本版本以 NM_ALARM_CDR（告警汇总表）为核心进行同步，而非 NM_ALARM_EVENT。
-- - ALARM_INST_ID 作为 NM_ALARM_SYNC_STATUS 表的唯一标识（核心关联键）
-- - EVENT_INST_ID 仅用于记录最新的告警事件（非必填）
-- ============================================================================
...
-- 架构: 以 ALARM_INST_ID（NM_ALARM_CDR）为核心关联键
-- ============================================================================
CREATE TABLE NM_ALARM_SYNC_STATUS (
    SYNC_ID           NUMBER(12)      NOT NULL,    -- 同步记录唯一标识
    ALARM_INST_ID     NUMBER(13)      NOT NULL,    -- 关联的告警汇总ID (NM_ALARM_CDR.ALARM_INST_ID) [核心关联键]
    EVENT_INST_ID     NUMBER(13),                  -- 关联的告警事件ID (NM_ALARM_EVENT.EVENT_INST_ID) [可选，记录最新事件]
    ...
    CONSTRAINT UK_SYNC_ALARM_INST UNIQUE (ALARM_INST_ID)   -- 以 ALARM_INST_ID 为唯一约束
);

-- 同步状态表字段注释
COMMENT ON TABLE NM_ALARM_SYNC_STATUS IS '告警同步状态跟踪表（v2.0），以NM_ALARM_CDR.ALARM_INST_ID为核心，记录每条ZMC告警与Prometheus/Alertmanager同步的状态';
COMMENT ON COLUMN NM_ALARM_SYNC_STATUS.SYNC_ID IS '同步记录唯一标识，由序列SEQ_ALARM_SYNC_STATUS生成';
COMMENT ON COLUMN NM_ALARM_SYNC_STATUS.ALARM_INST_ID IS '[核心关联键] 关联NM_ALARM_CDR表的告警汇总ID，唯一标识一个告警源';
COMMENT ON COLUMN NM_ALARM_SYNC_STATUS.EVENT_INST_ID IS '关联NM_ALARM_EVENT表的告警事件ID，记录最新的告警事件（非必填）';
...
-- 同步状态表索引
CREATE INDEX IDX_SYNC_STATUS_ALARM_INST ON NM_ALARM_SYNC_STATUS(ALARM_INST_ID);  -- 核心关联键索引
...
INSERT INTO NM_ALARM_LABEL_MAPPING (MAPPING_ID, SOURCE_FIELD, TARGET_LABEL, TRANSFORM_TYPE, IS_ENABLED, LABEL_TYPE, SORT_ORDER) VALUES
(SEQ_ALARM_LABEL_MAPPING.NEXTVAL, 'ALARM_INST_ID', 'alarm_id', 'DIRECT', 'Y', 'LABEL', 8);  -- v2.0: 告警汇总ID（核心标识）
INSERT INTO NM_ALARM_LABEL_MAPPING (MAPPING_ID, SOURCE_FIELD, TARGET_LABEL, TRANSFORM_TYPE, IS_ENABLED, LABEL_TYPE, SORT_ORDER) VALUES
(SEQ_ALARM_LABEL_MAPPING.NEXTVAL, 'EVENT_INST_ID', 'event_id', 'DIRECT', 'N', 'LABEL', 9);  -- v2.0: 事件ID降级为可选
INSERT INTO NM_ALARM_LABEL_MAPPING (MAPPING_ID, SOURCE_FIELD, TARGET_LABEL, TRANSFORM_TYPE, IS_ENABLED, LABEL_TYPE, SORT_ORDER) VALUES
(SEQ_ALARM_LABEL_MAPPING.NEXTVAL, 'ENVIRONMENT', 'env', 'DIRECT', 'Y', 'LABEL', 10);
INSERT INTO NM_ALARM_LABEL_MAPPING (MAPPING_ID, SOURCE_FIELD, TARGET_LABEL, TRANSFORM_TYPE, IS_ENABLED, LABEL_TYPE, SORT_ORDER) VALUES
(SEQ_ALARM_LABEL_MAPPING.NEXTVAL, 'RES_INST_TYPE', 'resource_type', 'DIRECT', 'Y', 'LABEL', 11);
